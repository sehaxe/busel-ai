"""
🧪 busel SHPAK 5-RUN PROFILER — v5.8 research validation
Compares baseline vs Sparse-BitNet 6:8, GradLite, LCSB, and all-combined on shpak 52.8M.
Per run: 2 warmup + 10 measured steps, batch=128 ctx=4096, d_model=384 n_layers=8.
"""
import os
...
def main():
    ...
    runs = [
        ...
        ("3. + GradLite error fb", {"use_error_feedback": True}),
        ("4. + LCSB ratio=0.5", {"selective_backward": True, "backward_ratio": 0.5}),
    ...
    print("\n" + "=" * 80)
    print("📊 SHPAK 5-RUN COMPARISON (52.8M, batch=16 ctx=4096, 10 steps)".center(80))
    print("=" * 80)
    print(f"{'Run':<35} | {'Step (ms)':>10} | {'Peak (MB)':>10} | {'tok/s':>8} | {'Loss@10':>8}")
    print("-" * 80)
    base = results[0]
    for r in results:
        if "error" in r:
